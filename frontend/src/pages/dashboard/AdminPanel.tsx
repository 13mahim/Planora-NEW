...
export function AdminPanel() {
  ...
  const [filter, setFilter] = useState<"all" | "featured" | "admin">("all");
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          { label: "Total Events", value: events.length, color: "bg-blue-50 text-blue-600", icon: Calendar, onClick: () => { setTab("events"); setFilter("all"); } },
          { label: "Total Users", value: users.length, color: "bg-orange-50 text-orange-600", icon: Users, onClick: () => { setTab("users"); setFilter("all"); } },
        ...
        <div className="bg-white rounded-[2.5rem] border border-neutral-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-neutral-100 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {filter === "featured" && (
                <button onClick={() => setFilter("all")} className="text-xs text-orange-600 font-bold hover:underline">Show All</button>
              )}
              <span className="text-xs font-bold text-neutral-400 uppercase tracking-widest">
                {(filter === "featured" ? events.filter((e: any) => e.isFeatured) : events).length} Total
              </span>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[2.5rem] border border-neutral-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-neutral-100 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-bold text-neutral-400 uppercase tracking-widest">
                {(filter === "admin" ? users.filter(u => u.role === "admin") : users).length} Total
              </span>
            </div>
          </div>
          <div className="divide-y divide-neutral-100 max-h-[500px] overflow-y-auto">
