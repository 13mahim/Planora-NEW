    ...
    <form onSubmit={onSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="md:col-span-2 space-y-2">
          ...
          <input type="text" required value={formData.title} onChange={(e) => setFormData(p => ({ ...p, title: e.target.value }))} placeholder="e.g. Annual Tech Summit" className={inputCls} />
        </div>
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <input type="date" required value={formData.date} onChange={(e) => setFormData(p => ({ ...p, date: e.target.value }))} className={`${inputCls} pl-12`} />
          </div>
        </div>
        <div className="space-y-2">
          <label className="text-xs font-bold text-neutral-400 uppercase tracking-widest ml-1">Time</label>
          <input type="time" required value={formData.time} onChange={(e) => setFormData(p => ({ ...p, time: e.target.value }))} className={inputCls} />
        </div>
        <div className="md:col-span-2 space-y-2">
          ...
          <div className="relative">
            ...
            <input type="text" required value={formData.venue} onChange={(e) => setFormData(p => ({ ...p, venue: e.target.value }))} placeholder="Physical address or online link" className={`${inputCls} pl-12`} />
          </div>
        </div>
        <div className="md:col-span-2 space-y-2">
          <label className="text-xs font-bold text-neutral-400 uppercase tracking-widest ml-1">Description</label>
          <textarea rows={4} required value={formData.description} onChange={(e) => setFormData(p => ({ ...p, description: e.target.value }))} placeholder="Tell people what your event is about..."
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => setFormData(p => ({ ...p, isPublic: true }))}
              className={cn("grow py-3 rounded-xl border flex items-center justify-center gap-2 font-semibold transition-all", formData.isPublic ? "bg-orange-50 dark:bg-orange-900/30 border-orange-500 text-orange-600" : "bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 text-neutral-400")}>
              <Globe size={16} /> Public
            </button>
            <button type="button" onClick={() => setFormData(p => ({ ...p, isPublic: false }))}
              className={cn("grow py-3 rounded-xl border flex items-center justify-center gap-2 font-semibold transition-all", !formData.isPublic ? "bg-orange-50 dark:bg-orange-900/30 border-orange-500 text-orange-600" : "bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 text-neutral-400")}>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => setFormData(p => ({ ...p, isFree: true, registrationFee: 0 }))}
              className={cn("grow py-3 rounded-xl border flex items-center justify-center font-semibold transition-all", formData.isFree ? "bg-emerald-50 dark:bg-emerald-900/30 border-emerald-500 text-emerald-600" : "bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 text-neutral-400")}>
              Free
            </button>
            <button type="button" onClick={() => setFormData(p => ({ ...p, isFree: false }))}
              className={cn("grow py-3 rounded-xl border flex items-center justify-center font-semibold transition-all", !formData.isFree ? "bg-orange-50 dark:bg-orange-900/30 border-orange-500 text-orange-600" : "bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 text-neutral-400")}>
              Paid
            </button>
          </div>
        </div>
        {!formData.isFree && (
          <div className="md:col-span-2 space-y-2">
            ...
            <div className="relative">
              ...
              <input type="number" required min="1" step="0.01" value={formData.registrationFee} onChange={(e) => setFormData(p => ({ ...p, registrationFee: parseFloat(e.target.value) }))} className={`${inputCls} pl-12`} />
            </div>
          </div>
        )}
      </div>
...
const defaultForm: FormData = { title: "", description: "", date: "", time: "", venue: "", isPublic: true, isFree: true, registrationFee: 0, category: "General" };
...
export function MyEvents() {
  ...
  const fetchEvents = async () => {
    try { const data = await eventService.getMyEvents(); setEvents(data); }
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchEvents();
    } catch (err: any) { toast.error(err.response?.data?.message || "Failed to update event"); }
    finally { setSubmitting(false); }
  };
  ...
  const openParticipants = async (event: Event) => {
    setSelectedEvent(event);
    try { const data = await eventService.getParticipants(event.id); setParticipants(data); setShowParticipantsModal(true); }
  ...
  const handleParticipantAction = async (participantId: string, status: "approved" | "rejected" | "banned") => {
    try {
      ...
      if (selectedEvent) { const data = await eventService.getParticipants(selectedEvent.id); setParticipants(data); }
    } catch { toast.error("Failed to update participant"); }
  };

  const modalCls = "relative w-full max-w-2xl bg-white dark:bg-neutral-900 rounded-[2.5rem] shadow-2xl overflow-hidden";
  const modalHeaderCls = "p-8 border-b border-neutral-100 dark:border-neutral-800 flex justify-between items-center";
  const closeBtnCls = "p-2 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-full text-neutral-600 dark:text-neutral-300";

  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-neutral-900 dark:text-white">My Events</h1>
          <p className="text-neutral-500 dark:text-neutral-400 mt-1">Manage the events you've created and organized.</p>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-64 bg-neutral-100 dark:bg-neutral-800 rounded-[2.5rem] animate-pulse" />)}
        </div>
      ) : events.length === 0 ? (
        <div className="text-center py-24 bg-white dark:bg-neutral-900 rounded-[3rem] border border-dashed border-neutral-300 dark:border-neutral-700">
          ...
          <p className="text-neutral-500 dark:text-neutral-400">Create your first event to get started.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {events.map((event) => (
            <div key={event.id} className="bg-white dark:bg-neutral-900 p-8 rounded-[2.5rem] border border-neutral-100 dark:border-neutral-800 shadow-sm hover:shadow-xl transition-all duration-500">
              <div className="flex justify-between items-start mb-6">
                <div className="w-12 h-12 bg-orange-100 dark:bg-orange-900/30 rounded-2xl flex items-center justify-center text-orange-600"><CalendarIcon size={24} /></div>
                <div className="flex gap-2">
                  <span className={cn("px-3 py-1 rounded-full text-[10px] font-bold uppercase", event.isPublic ? "bg-blue-50 text-blue-600" : "bg-purple-50 text-purple-600")}>{event.isPublic ? "Public" : "Private"}</span>
                  <span className={cn("px-3 py-1 rounded-full text-[10px] font-bold uppercase", event.isFree ? "bg-emerald-50 text-emerald-600" : "bg-orange-50 text-orange-600")}>{event.isFree ? "Free" : formatCurrency(event.registrationFee)}</span>
                </div>
              </div>
              <h3 className="text-xl font-bold text-neutral-900 dark:text-white mb-2">{event.title}</h3>
              <div className="space-y-2 mb-6">
                <div className="flex items-center gap-2 text-neutral-500 dark:text-neutral-400 text-sm"><CalendarIcon size={14} className="text-orange-600" /><span>{event.date} · {event.time}</span></div>
                <div className="flex items-center gap-2 text-neutral-500 dark:text-neutral-400 text-sm"><MapPin size={14} className="text-orange-600" /><span>{event.venue}</span></div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => openParticipants(event)} className="flex items-center gap-1 px-4 py-2 bg-neutral-50 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 font-bold rounded-xl hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-all text-sm"><Users size={16} /><span>Participants</span></button>
                <button onClick={() => openEdit(event)} className="flex items-center gap-1 px-4 py-2 bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 font-bold rounded-xl hover:bg-neutral-800 dark:hover:bg-neutral-100 transition-all text-sm"><Edit2 size={16} /><span>Edit</span></button>
                <button onClick={() => handleDelete(event.id)} className="p-2 bg-neutral-50 dark:bg-neutral-800 text-neutral-400 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-500 transition-all"><Trash2 size={18} /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <AnimatePresence>
        {showCreateModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className={modalCls} onClick={(e) => e.stopPropagation()}>
              <div className={modalHeaderCls}>
                ...
                <button onClick={() => setShowCreateModal(false)} className={closeBtnCls}><X size={24} /></button>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className={modalCls} onClick={(e) => e.stopPropagation()}>
              <div className={modalHeaderCls}>
                ...
                <button onClick={() => setShowEditModal(false)} className={closeBtnCls}><X size={24} /></button>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className={modalCls} onClick={(e) => e.stopPropagation()}>
              <div className={modalHeaderCls}>
                ...
                <button onClick={() => setShowParticipantsModal(false)} className={closeBtnCls}><X size={24} /></button>
              </div>
